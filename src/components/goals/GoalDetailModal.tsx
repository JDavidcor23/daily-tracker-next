...
interface GoalDetailModalProps {
  ...
  onLinkMultipleTasks: (goalId: string, todoIds: string[]) => void;
...
export const GoalDetailModal: React.FC<GoalDetailModalProps> = ({
  ...
  const [searchMode, setSearchMode] = useState<'task' | 'tag'>('task');
  ...
  const handleLinkByTag = (e: React.FormEvent) => {
    e.preventDefault();
    if (!selectedTagId) return;
    const todoIds = todosOfSelectedTag.map(t => t.id);
    if (todoIds.length === 0) return;
    onLinkMultipleTasks(goal.id, todoIds);
    setSelectedTagId('');
  };
  ...
  // Extract unique tags from unlinked todos
  const availableTags = React.useMemo(() => {
    const unlinkedTodos = allTodos.filter(todo => !goal.linkedTodos.some(lt => lt.id === todo.id));
    const tagMap = new Map<string, any>();
    unlinkedTodos.forEach(todo => {
      todo.tags?.forEach(tag => {
        tagMap.set(tag.id, tag);
      });
    });
    return Array.from(tagMap.values());
  }, [allTodos, goal.linkedTodos]);

  const filteredTags = availableTags.filter(tag => 
    tag.name.toLowerCase().includes(tagSearchTerm.toLowerCase())
  );

  const todosOfSelectedTag = React.useMemo(() => {
    if (!selectedTagId) return [];
    return allTodos
      .filter(todo => !goal.linkedTodos.some(lt => lt.id === todo.id))
      .filter(todo => todo.tags?.some(tag => tag.id === selectedTagId));
  }, [allTodos, goal.linkedTodos, selectedTagId]);
              ...
              <div className="space-y-4">
                ...
                {searchMode === 'task' ? (
                  ...
                  <div className="space-y-2 animate-in slide-in-from-right-2 duration-200">
                    ...
                    <form onSubmit={handleLinkByTag} className="flex gap-2">
                      <select 
                        className="flex-1 bg-slate-50 dark:bg-slate-800 border-none rounded-xl px-4 py-2.5 text-sm dark:text-slate-100 focus:ring-2 focus:ring-brand-500/20 font-bold"
                        value={selectedTagId}
                        onChange={(e) => setSelectedTagId(e.target.value)}
                        style={{ color: availableTags.find(t => t.id === selectedTagId)?.color }}
                      >
                        <option value="" className="text-slate-500 font-normal">
                          {filteredTags.length === 0 
                            ? 'No tags found' 
                            : `Select tag (${filteredTags.length} found)...`}
                        </option>
                      ...
                      <button 
                        type="submit" 
                        disabled={!selectedTagId || todosOfSelectedTag.length === 0}
                        className="bg-brand-600 text-white px-4 rounded-xl text-sm font-bold disabled:opacity-30 disabled:cursor-not-allowed hover:bg-brand-500 active:scale-95 transition-all flex items-center gap-2"
                      >
                        Link All {todosOfSelectedTag.length > 0 && `(${todosOfSelectedTag.length})`}
                      </button>
                    </form>
                    {selectedTagId && (
                      <div className="px-2 py-1 flex flex-wrap gap-1">
                        {todosOfSelectedTag.map(todo => (
                          <span key={todo.id} className="text-[10px] bg-slate-100 dark:bg-slate-800 text-slate-500 px-2 py-0.5 rounded-full">
                            {todo.text}
                          </span>
                        ))}
                      </div>
